...
\begin{frame}{Correctness Conditions for WMC (2/2)}
  ...
  \item there is no clause \structure{$c \in \phi$} such that \structure{$\neg p
      \in c$},
  \item if \structure{$\{ p \} \in \phi$}, then this is the only clause that
    mentions \structure{$p$},
  \item and for any \structure{$c, d \in \phi$} such that \structure{$c \ne d$},
    \structure{$p \in c$} and \structure{$p \in d$},
    \[
