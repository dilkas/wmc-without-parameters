...
% TODO (on encodings and clauses): I realise the paper is already long but it
% might be useful to have a quick example
% TODO: If I am reading this correctly you are making a stark statement about
% the inefficacy. It might be good to rephrase this differently. You might want
% to say that there are many corner cases where there are no improvements. 
% TODO: Is there a reference for this example? If yes provide one. If no, maybe
% you could provide a link to the output, or alternatively, you could say
% something like the reader is free to check that such an example produces no
% improvements
% TODO: is this example the same as the previous one?
...
\end{proposition}
% TODO: It’s a bit strange to say same answer, when you don’t precisely refer to
% this anywhere. One option is to say the above is the WMC instance and let
% Delta be the weighted model count. Then the new encoding Has a weighted model
% count of delta as well. Or something like that.
% But looking at the text below, I see that you use this phrasing everywhere,
% and I admit my formulation is awkward. So maybe the best compromise here is
% just simply add a footnote explaining what exactly you mean when you say same
% answer
% TODO: If it is the case that you cannot include the proofs in line, and indeed
% it might be better for readability to have an appendix with all the proofs, it
% might make sense to provide the type of argument you are using in the proofs
% so that readers know what to expect when they look into it
\begin{proof}
...
\begin{theorem}[Correctness for WMC] \label{thm:correctness}
  ...
  w)$, returns a PBP instance with the same answer, provided \emph{either} of
...
\begin{proof}
  ...
  show that % TODO: footnote: explain how
...
% TODO: consider moving (some) proofs to the appendix or at least making a clear
% argument why they should be in the text.
