...
% TODO: we also show correctness
% TODO: and new notation!
...
NOTE: Adding scaling factor $\omega$ to the definition allows us to remove
...
\paragraph{TODO}
\begin{itemize}
\item Define the terminology surrounding BNs: variables, values, directed
  probabilistic graphical model, DAG, binary = two values per variable.
\item Reuse this term where I'm referring to two values.
\item Give a brief description of each encoding.
\item Talk about the parameter/indicator variable distinction.
\end{itemize}
...
authors and the year of publication: \texttt{d02}
\cite{DBLP:conf/kr/Darwiche02}, \texttt{sbk05} \cite{DBLP:conf/aaai/SangBK05},
\texttt{cd05} \cite{DBLP:conf/ijcai/ChaviraD05}, \texttt{cd06}
\cite{DBLP:conf/sat/ChaviraD06}, \texttt{bklm16} \cite{DBLP:conf/ecai/BartKLM16}.

\begin{remark}
  Strictly speaking, encodings such as \texttt{cd05} and \texttt{cd06} are
  \emph{not} WMC encodings. Instead, they encode Bayesian network inference into
  instances of the \emph{minimum-cardinality} WMC problem, where the answer is
  defined to be $\sum_{Y \models \phi\text{, }|Y| = k} \prod_{Y \models l}
  w(l)$, where $k = \min_{Y \models \phi\text{, }Y \ne \emptyset} |Y|$, if $k$
  exists, otherwise the answer is zero. This additional condition on model
...
\end{proposition}
\begin{proof} % TODO: equation is not the right word
  Let $f = \prod_{c \in \phi} [c]_0^1$, and $g = \prod_{x \in X_I \cup X_P}
  [x]_{w(\neg x)}^{w(x)}$. Then the answer of \cref{eq:new_wmc} is
  \[
...
\item Experimental research questions:
  ...
  \end{itemize}
\item TODO: How many times am I faster than old cd06 and new bklm16? Perhaps add
  other numbers as well.
\item To perform Bayesian network inference with \textsf{DPMC} (or with any
...
\begin{figure}
  ...
  \label{fig:box}
\end{figure} % TODO: maybe add sbk05 with just 'before'?
