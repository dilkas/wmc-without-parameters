...
% NOTE: What is the problem? Why is it interesting and important?
...
% NOTE: What's wrong with previous proposed solutions? How does mine differ?
...
algorithms. Our work is similar in that it introduces a new representation for
computational problems but is able to reuse recent WMC algorithms based on
pseudo-Boolean function manipulation, namely, \textsf{ADDMC}
\cite{DBLP:conf/aaai/DudekPV20} and \textsf{DPMC} \cite{DBLP:conf/cp/DudekPV20}.
Furthermore, we identify necessary conditions for transforming a WMC instance
into the new format. As many WMC inference algorithms such as
...
could be via direct compilation to a more convenient representation. However,
while direct compilation of Bayesian networks to SDDs has been investigated
\cite{DBLP:conf/ecsqaru/ChoiKD13}, SDDs are not expressive enough to avoid the
issue.

% NOTE: What are the key components of my approach and results? Also include any
% specific limitations. What about your project is new or different from
% existing solutions? List of refutable contributions (with forward references
% to the rest of the paper) What is the outcome of the project?
In this work, we introduce a way to transform WMC problems to a new format based
on pseudo-Boolean functions---\emph{pseudo-Boolean projection} (PBP). We
...
\cite{DBLP:conf/ecai/BartKLM16,DBLP:conf/ijcai/ChaviraD05,DBLP:conf/sat/ChaviraD06,DBLP:conf/kr/Darwiche02,DBLP:conf/aaai/SangBK05}
can indeed be simplified in this manner. This transformation enables two
encodings that were previously incompatible with most WMC algorithms (due to
using a different definition of WMC
\cite{DBLP:conf/ijcai/ChaviraD05,DBLP:conf/sat/ChaviraD06}) to be run with
\textsf{ADDMC} and \textsf{DPMC} and results in a significant performance boost
for one other encoding, allowing it to surpass the current state of the art.
Finally, our theoretical contributions result in a convenient algebraic way of
reasoning about two-valued pseudo-Boolean functions and position WMC encodings
on a common ground, identifying their key properties and assumptions.

%To the best of the authors' knowledge, none of these compilation/inference
%techniques have a publicly available implementation.
...
\begin{definition}[WMC]
  ...
  X_I \cup X_P\} \to \mathbb{R}$ is the weight function. The \emph{answer} of
  the instance is $\sum_{Y \models \phi} \prod_{Y \models l} w(l)$.
\end{definition}
...
generated by \textsf{Ace}, then variable types are explicitly identified in the
LMAP file generated alongside the encoding. Otherwise, we define a variable $x$
to be a parameter variable if $w(x) = w(\neg x) = 1$. Next, we formally define a
variation of the WMC problem used by some of the Bayesian network encodings
...
\begin{definition}[Minimum-Cardinality WMC]
  ...
  instance, but its answer is defined to be $\sum_{Y \models \phi\text{, }|Y| =
    k} \prod_{Y \models l} w(l)$ (where $k = \min_{Y \models \phi} |Y|$) if
  $\phi$ is satisfiable, and zero otherwise.
\end{definition}
...
% TODO: mention that $2^x$ denotes the powerset of $X$.

In this work we propose a more expressive representation for WMC based on
pseudo-Boolean functions. Pseudo-Boolean functions, most commonly represented as
...
we introduce some notation. For any propositional formula $\phi$ over a set of
...
% TODO: mention where the operations are from
% TODO: should I consider different domains?
% TODO: can I say that a domain automatically shrinks (w.r.t. x) if f|x=0 =
% f|x=1?
...
\begin{definition}[Operations] \label{def:operations}
  ...
  inherit properties such as associativity and commutativity. By regarding a
  real number as a constant pseudo-Boolean function, we can reuse the same
  definitions to define \emph{scalar} operations as
  \[
...
Finally, below we list some properties of the operations on pseudo-Boolean
functions discussed in this section that can be conveniently represented using
our syntax. The proofs of all these properties follow directly from the
definitions.
...
\begin{theorem}[Correctness for WMC] \label{thm:correctness}
  ...
  \item and either
    ...
    \item for all parameter variables $p \in X_P$, there is a non-empty family
      of literals $(l_i)_{i=1}^n$ such that \label{cond:d02}
      \begin{enumerate}
      \item $w(\neg p) = 1$,
      ...
      \end{enumerate}
    \item or for all parameter variables $p \in X_P$, \label{cond:bklm16}
      \begin{enumerate}
      \item $w(p) + w(\neg p) = 1$,
...
\Cref{cond:one} applies to all four WMC encodings under consideration and
ensures that only parameter variables carry nontrivial weights. \Cref{cond:d02}
...
literals. \Cref{cond:bklm16} is for encodings that have implications rather than
...
\begin{proof}
  ...
  \cref{def:new_wmc}, its answer is $\left(\exists_{X_I \cup X_P} \left(\prod_{c
  ...
  \]
  and $\omega = 1$, and so we want to show that $\left[ \bigwedge_{i = 1}^n l_i
  \right]_1^{w(p)} = \exists_p [p]_1^{w(p)} \left[ p \Leftrightarrow
    \bigwedge_{i=1}^n l_i \right]_0^1$, and indeed
  \[
  ...
  Then $\omega = 1$, and $F_p = \left\{ \left[\bigwedge_{l \in c \setminus \{ p
        \}} \neg l\right]_1^{w(p)} \;\middle|\; c \in \phi_p \right\}$, so we
  need to show that
  \[
...
% We assume that $\omega = 1$ (i.e., $\{ p \} \not\in \phi$ for all parameter
% variables $p \in X_P$) as the proof can be easily extended to handle $\omega
% \neq 1$ similarly to the proof of \cref{thm:correctness}. 
...
inefficient.
% TODO: mention (somewhere) that we can't compare to Gogate's work and direct
% compilation (take the text from the introduction)

\section{Conclusion} % So what?
