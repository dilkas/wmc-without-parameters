...
\paragraph{TODO}
\begin{itemize}
\item Explain two meanings of entailment in the definition of WMC.
\end{itemize}

Throughout the paper, we use set theoretic notation for many concepts in logic.
We identify a model with a set of variables that correspond to the positive
literals in the model (and all other variables are the negative literals of the
model). A formula in CNF is a set of clauses that are part of an implicit
conjunction. Similarly, a clause is a set of literals that are part of an
implicit disjunction. For example, let $\phi = (\neg a \lor b) \land a$ be a
propositional formula over variables $a$ and $b$. Then an equivalent
set-theoretic representation of $\phi$ is $\{ \{ \neg a, b \}, \{ a \} \}$. Any
subset of $\{ a, b \}$ is an interpretation of $\phi$, e.g., $\{ a, b \}$ is a
model of $\phi$ (written $\{ a, b \} \models \phi$), while $\emptyset$ is an
interpretation but not a model.
...
\begin{theorem}[Correctness for Minimum-Cardinality WMC]
  ...
  \item all models of $\{ c \in \phi \mid c \cap X_P = \emptyset \}$ (as subsets
    of $X_I$) have the same cardinality;
...
\end{theorem}
\begin{proof} % TODO: introduce the variables that come from the algorithm
  We split the proof into two parts. Let
  \[
    ...
    \right\} \cup \left\{ \left[ \bigwedge_{l \in c \setminus \{ p \}} \neg l
      ...
      \text{, } c \ne \{ p \} \right\}
  \]
  be as returned by \cref{alg:transformation}. In the first part, we show that
  there is a bijection between minimum-cardinality models of $\phi$ and $Y
  \subseteq X_I$ such that $\left(\prod_{f \in F} f\right)(Y) \ne 0$ (with the
  assumption that $w(p) \ne 0$ for all $p \in X_P$).\footnote{The assumption is
    only there to make the proof more convenient and can be omitted by
    reformulating the proof in a slightly more cumbersome way.} Let $Y \subseteq
  X_I$ and $Y' \subseteq X_I \cup X_P$ be related via this bijection. Then in
  the second part we will show that
  \[
    \prod_{Y' \models l} w(l) = \left(\prod_{f \in F} f\right)(Y).
  \]

  On the one hand, if $Y \subseteq X_I \cup X_P$ is a minimum-cardinality model
  of $\phi$, then
  \[
    \left(\prod_{f \in F}\right)(Y \cap X_I) \ne 0
  \]
  ...
  need to show that $Z$ is minimum cardinality.
\end{proof} % TODO: first part
% TODO: make a separate definition for minimum cardinality model and use it in
% the definition of MCWMC
% TODO: use the MCWMC abbreviation
% TODO: my current proof doesn't use omega. Maybe just say that the proof is
% easily extendable to phi s.t. omega != 1 similarly to the proof of
% thm:correctness.

\section{Experimental Evaluation (mostly done)}
% TODO: define cardinality of model
