...
% TODO: check if each condition is actually used. Maybe turn this into a
% paragraph that gives an overview of the proof.
\begin{proof}
...
% NOTE: benefits of having the proof in the paper:
% 1) it puts all the encodings on a common ground
% 2) it illustrates the convenience of our notation for reasoning about (certain
% types of) pseudo-Boolean functions (stress this!!!)
% 3) it's too big and too important to be left for the appendix
% NOTE: we don't make a distinction between a real number and a (PB) function
% that always returns that number.
% NOTE: projection is assumed to have lowest precedence (i.e., \exists_x fg =
% \exists_x (fg) and not (\exists_x f)g).
% TODO: need another basic property: (fg)|_{x=i} = f|_{x=i} * g|_{x=i} for i=0, 1.
% TODO: \times -> \cdot
