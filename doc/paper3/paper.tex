...
\begin{abstract}
  ...
  eliminating around \SI{57}{\percent} of variables on average across various
...
computational problems but can reuse recent WMC algorithms based on
...
issue. Furthermore, to the best of the authors' knowledge, neither
compilation/inference technique has a publicly available implementation.
...
on common ground, identifying their key properties and assumptions.
...
\begin{definition}[WMC]
  ...
  X_I \cup X_P\} \to \mathbb{R}$ is the weight function. Its \emph{answer} is
...
\begin{definition}[PBP Instance] \label{def:new_wmc}
  ...
  $\omega \in \mathbb{R}$ is the scaling factor. Its \emph{answer} is $\omega
  \cdot \left(\exists_X\prod_{f \in F}f\right)(\emptyset)$.
\end{definition}
...
\begin{proof}
  ...
  [x]_{w(\neg x)}^{w(x)}$. Then the WMC answer is
  \eqref{eq:new_wmc} is
  \[
...
(for \texttt{d02}) simply states that each parameter variable is equivalent to a
conjunction of indicator literals. \Cref{cond:bklm16} is for encodings that have
implications rather than equivalences associated with parameter variables
(which, in this case, is \texttt{bklm16}). It ensures that each clause has at
most one positive parameter literal and no negative ones, and that at most one
implication clause per any parameter variable $p \in X_P$ can `force $p$ to be
positive'.
...
\begin{proof}
  ...
  \left[ p \Leftrightarrow \bigwedge_{i=1}^n l_i \right]_0^1$, and indeed
  \[
