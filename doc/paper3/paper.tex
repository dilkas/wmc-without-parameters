...
% TODO: Check author % instructions in the lncs/ directory.
% TODO: Make sure that the paper fits into 15 pages.

\title{Weighted Model Counting Without Parameter Variables}

\author{Paulius Dilkas\inst{1} \and
Vaishak Belle\inst{1,2}}
% TODO: maybe add an envelope or two?
...
heuristics.
% TODO: While the complexity results in the cited papers [2, 32] have various
% widths as the dominant (i.e., exponential) part of the formula, the number of
% variables is featured as well, e.g., as $n$ in [2]. We did not notice a
% significant difference in treewidth before and after performing our
% transformation (we can include details about this in the final version of the
% paper). The observed performance improvement is more likely related to the
% variable ordering heuristic used by ADDs.
% One important aspect that is not analyzed is how the technique affects the
% treewidth of the instance.
% This is important because DPMC makes use of tree decompositions of low width.
% In particular, from a theoretical standpoint the questions are whether the
% given reduction can increase or decrease the treewidth, and if yes, are there
% any bounds to it.
% (to me it seems that the reduction can decrease treewidth but cannot increase
% it)
% From practical standpoint the question is how the widths of the tree
% decompositions used by DPMC+bklm16 and DPMC+bklm16++ compare.
% Do you have any data on this?
% Also related to treewidth, the sentence "As the parameterised complexity of
% model counting (and, by extension, WMC) depends on the number of variables [2,
% 32], ..." sounds very weird because the cited papers consider the complexity
% parameterized by treewidth.
% Should "number of variables" be replaced by "treewidth"?
...
% TODO: We chose to transform existing encodings instead of creating a new one
% because encodings such as cd05, cd06, and bklm16 do a good job at reducing
% each CPT to a minimal logical representation by using techniques such as prime
% implicants and limited forms of resolution. We wanted to reuse and build on
% these ideas.
...
experiments because they never looked at numerical answers.
...
% TODO: Examples can be added for illustration of the transformation algorithm.
% TODO:  We will update the camera-ready version of the paper with a complete
% description of how the input format was adjusted to support a new type of
% problem instance. In short, we replace the weight lines of the form 'w
% <variable> <positive weight>' with 'w <conjunction of literals> <positive
% weight> <negative weight>'. The input format is restricted to conjunctions for
% simplicity, but, of course, the algorithm can accept arbitrary formulas.
\begin{algorithm}[t]
...
out of the five Bayesian network encodings: \texttt{bklm16}
\cite{DBLP:conf/ecai/BartKLM16}, \texttt{cd05}
...
with \texttt{sbk05} because the roles of indicator and parameter (i.e.,
...
\begin{proof}
  ...
  need to show that $\prod_{c \in \phi_p} \left[\bigwedge_{l \in c \setminus \{
      p \}} \neg l\right]_1^{w(p)} = \exists_p [p]_{1-w(p)}^{w(p)} \prod_{c \in
    \phi_p} [c]_0^1$. We can rearrange the right-hand side as
  \begin{align*}
...
% TODO: should I add something for him as well?
