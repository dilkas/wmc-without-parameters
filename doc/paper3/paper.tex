...
\paragraph{Notes.}
\begin{itemize}
\item \Cref{fig:cumulative} shows:
  \begin{itemize}
  \item $\textsf{DPMC}+\texttt{bklm16++}$ remains the best-performing
    combination across all time limits up to \SI{1000}{\second} with $\textsf{Ace} +
    \texttt{cd06}$ and $\textsf{DPMC}+\texttt{bklm16}$ not far behind;
  \item $\textsf{DPMC}+\texttt{bklm16++}$ to be 3.35 times faster than
    $\textsf{DPMC}+\texttt{bklm16}$ and 2.96 times faster than
    $\textsf{Ace}+\texttt{cd06}$.
  \end{itemize}
\item \Cref{tbl:performance} further shows that
...
\item TODO: now talk about \cref{fig:scatter}.
\item We also record numbers of variables in each encoding before and after our
  additional transformation.
\item TODO: describe \cref{fig:box}.
\item Indeed, the numbers of variables in \texttt{bklm16++} and \texttt{d02++}
  encodings are equal for binary Bayesian networks (i.e., most of our data).
  Nonetheless, \texttt{bklm16++} performs much faster than \texttt{d02++}.
\end{itemize}
% TODO: Describe the results. Make sure to answer all of the listed questions.
% TODO: double check that the numbers in the table make sense.
