...
WMC has emerged as a powerful computational framework for problems in many
domains, e.g., probabilistic graphical models such as Bayesian networks and
Markov networks
...
% TODO: Our work is similar to \cite{DBLP:conf/uai/GogateD10} who suggest using
% a collection of soft (i.e., with weights) and hard clauses
% TODO: how different are the semantics?
% But:
% 1) we can handle arbitrary formulas and not just clauses
% 2) we assign two weights, not one
% 3) the semantics of these weights are completely different, similar to the
% ones from Markov networks. It is thus difficult to reuse the same formulation
% for a different WMC algorithm.
% 4) Since we present a WMC->PBP algorithm, we don't need to invent new
...
% NOTE: List of refutable contributions (with forward references to the rest of
% the paper) What is the outcome of the project?
\paragraph{Notes.}
\begin{itemize}
\item we also show correctness
\item and introduce a new notation
\item TODO: briefly mention what a pseudo-Boolean function is
\end{itemize}
...
% TODO: check if each condition is actually used
