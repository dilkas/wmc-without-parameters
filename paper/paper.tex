...
% TODO: Feedback: our empirical evaluations show a x% improvement
...
Intel Xeon Gold 6138 and Intel Xeon E5-2630 processors\footnote{Each instance is
run on the same processor for all encodings.} with an \SI{8}{\gibi\byte} memory
limit.
...
\end{itemize*}
% TODO: make sure to talk about the table (and mention that cd06 has its niche)
...
% 6. make sure that there are no duplicates in the bib files (look at LaTex output)

% TODO: check if the total checks out

\begin{table}
  \centering
  \caption{The numbers of instances (out of 1451) solved by ADDMC with each
    encoding (uniquely, faster than with others, and in total) under the
    described time and memory constraints}%
  \begin{tabular}{lrrr}
    \toprule
    Encoding & Unique & Fastest & Total \\
    \midrule
    \texttt{cd05} & 2 & 76 & 380 \\
    \texttt{cd06} & 0 & 214 & 363 \\
    \texttt{cw} & 234 & 633 & 965 \\
    \texttt{d02} & 40 & 58 & 782 \\
    \texttt{sbk05} & 12 & 16 & 750 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\item Our encoding is particularly promising on instances of Grid networks.
  ...
  query.
\item However, \texttt{cw} struggles with instances from Mastermind and
  ...
  simplifications that are exploited by \texttt{d02} but not \texttt{cw}.
\item An observation from the cumulative plot: \texttt{cw} solves the same
  number of instances in \SI{8.144}{\second} as \texttt{d02} does in
  \SI{1000}{\second}.
\end{itemize}
...
\Cref{tbl:asymptotes} shows how \texttt{cw} has both fewer variables and fewer
