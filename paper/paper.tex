...
\item However, \texttt{db20} struggles with instances from Mastermind and
  ...
  $\mathtt{db20}$.
\item An observation from the cumulative plot: \texttt{db20} solves the same
...
\todo[inline]{It could be confusing to use the word `variable' here. I'm using
  it as a synonym to `logical atom'.}
...
\Cref{tbl:asymptotes} shows how \texttt{db20} has both fewer variables and fewer
ADDs than any other encoding. However, note that these are upper bounds and most
encodings (including \texttt{db20}) can be smaller in certain situations (e.g.,
