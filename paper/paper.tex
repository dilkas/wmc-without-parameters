...
\todo[inline]{Define 'CPT'}

In line with the names of previous encodings, we shall call this encoding
\texttt{db20}. Let $V$ denote the set of random variables in a Bayesian network.
For any random variable $X \in V$, let $\mathrm{pa}(X)$ denote the set of
parents of $X$ and $\im X$ denote the set of possible values. We will use the
Bayesian network in \cref{fig:example_bn} as a running example throughout this
section. For this network, $V = \{ W, F, T \}$, $\mathrm{pa}(W) = \emptyset$,
