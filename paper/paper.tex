...
\todo[inline,caption={}]{
  Examples (must):
  \begin{itemize}
  \item an example of a measure that cannot be expressed using a factored weight
    function,
  \item an example of how to express it using conditional weight functions.
  \end{itemize}
  Also, define `conditional weight function'.
}
...
\todo[inline]{In line with the names of previous encodings, we shall call this
  encoding \texttt{db20}.}

A \emph{Bayesian network} is a directed acyclic graph with random variables as
vertices that defines a probability distribution over them. Let $V$ denote the
set of random variables and $X \in V$ be a random variable. Let $\im X$ denote
the set of possible values of $X$ and $\mathrm{pa}(X)$ denote the set of its
parents. The full probability distribution is then equal to
\begin{equation} \label{eq:factorisation}
  \prod_{X \in V} \Pr(X \mid \mathrm{pa}(X)).
\end{equation}
...
this network, $V = \{ W, F, T \}$, $\mathrm{pa}(W) = \emptyset$, $\mathrm{pa}(F)
= \mathrm{pa}(T) = \{ W \}$, $\im W = \im F = \{0, 1 \}$, and $\im T = \{ l, m,
h \}$.
...
\todo[inline]{Describe the algorithm and the CPT functions.}
...
\begin{figure}
  ...
  \begin{minipage}{0.59\textwidth}
    ...
    \captionof{figure}{Cumulative plot}
  \end{minipage}
  \begin{minipage}{0.39\textwidth}
    \centering
    \captionof{table}{Out of 1216 instances}%
    \begin{tabular}{lrrr}
      \toprule
...
\item Whenever a Bayesian network comes with an evidence file, we compute the
  probability of evidence. Otherwise, let $X$ denote the last-mentioned node in
  the Bayesian network. If $\mathsf{true}$ is a valid value of $X$, we compute
  the marginal probability of $X = \mathsf{true}$. Otherwise, we pick the value
  of $X$ which is listed first and calculate its marginal probability.
