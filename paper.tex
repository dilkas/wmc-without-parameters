...
\begin{definition}[\cite{DBLP:books/daglib/0090259,levasseur2012applied}]
  ...
  0$ is an atom if there is no $x \in \mathbf{B}$ such that $0 < x < a$. The BA
...
\begin{lemma}
  Let $\mathbf{B}$ be a BA, and let $F$ be the principal filter of $p \in
  \mathbf{B}$. Then, for any $a, b \in \mathbf{B}$, if $a/F = b/F$, then $a
  \land p = b \land p$.
\end{lemma}
\begin{proof}
  If $a/F = b/F$, then $a \to b \in F$ by \cref{def:quotient}. Then $p \le a \to
  b$ by \cref{def:filter}, and $(a \to b) \land p = p$ by the definition of
  $\le$. Then
  \[
    1 = p \lor \neg p = ((a \to b) \lor \neg p) \land (p \lor \neg p) = \neg a
    \lor b \lor \neg p = \neg (a \land p) \lor b.
  ...
  \[
    ...
    \land p) \lor b),
  \]
  and thus $b = (a \land p) \lor b$, and
  \[
    b \land p = (a \land p) \lor (b \land p) = (a \lor b) \land p.
  \]
  Similarly, $a \land p = (a \lor b) \land p$ which proves that $a \land p = b
  \land p$.
\end{proof}
...
\begin{proposition}[Measures on quotients] \label{def:measures_on_quotients}
  Let $\mathbf{B}$ be a BA with a measure $m : \mathbf{B} \to \mathbb{R}_{\ge
    0}$, and let $F$ be the principal filter of $p \in \mathbf{B}$. Let us
  define $m^*\colon \mathbf{B}/F \to \mathbb{R}_{\ge 0}$ as
  \[
    m^*(a/F) = m(a \land p)
  \]
  for any $a \in \mathbf{B}$. The function $m^*$ is well-defined. Furthermore,
  if $m$ is a WMC measure, then so is $m^*$.
\end{proposition}
