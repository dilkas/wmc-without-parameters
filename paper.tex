...
% TODO: Start here. Define filters (ref the undergraduate book), filter
% generation (same ref, but read it first), and quotients.
\begin{definition}
\end{definition}
...
\begin{lemma}
  Let $\mathbf{B}$ be a BA, and let $F$ be a filter in $\mathbf{B}$ generated by
  $p \in \mathbf{B}$. Then, for any $a, b \in \mathbf{B}$, if $a/F = b/F$, then
  $a \land p = b \land p$.
\end{lemma}
\begin{proof}
  % TODO: reference for a/F = b/F iff a <-> b \in F
  % TODO: reference for the definition of a filter
\end{proof}

\begin{definition}[Measures on quotients] \label{def:measures_on_quotients}
  Let $\mathbf{B}$ be a BA with a measure $m : \mathbf{B} \to \mathbb{R}_{\ge
    0}$, and let $F$ be a filter in $\mathbf{B}$ generated by some $p \in
  \mathbf{B}$. We can define a measure $m^*$ on $\mathbf{B}/F$ as
  \[
    m^*([a]) = m(a \land p).
  \]
\end{definition}
...
\begin{proposition}
  The function $m^*$ in \cref{def:measures_on_quotients} is well-defined.
  Furthermore, if $m$ is a WMC measure, then so is $m^*$.
\end{proposition}
\begin{proof}
\end{proof}
% TODO: prove that the two ways to compute the probability give the same answer.

% Feedback: Clarify the relationship between $\nWMC$ and the probability induced
% by a WMC formulation.
