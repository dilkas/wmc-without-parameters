...
\begin{proposition}[Measures on quotients] \label{def:measures_on_quotients}
  ...
  \begin{itemize}
  \item if $m$ is a probability measure, then so is $m^*$;
  \item if $m$ is strictly positive, then so is $m^*$;
  \item if $m$ is a WMC measure, then so is $m^*$.
  \end{itemize}
\end{proposition}
\begin{proof} % TODO: prove this
  \Cref{lemma:well-defined} proves that the function is well-defined.
\end{proof}
