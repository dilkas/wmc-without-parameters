...
\todo[inline]{
  Notation: make up my mind about $a, b$ vs. $x, y$ and stick to it (maybe $x,
  y$?).
}
...
\todo[inline]{Describe \cref{tbl:notation_example} as an example.}
\todo[inline]{Feedback: explain with examples: models $=$ atoms of algebra,
  formulas $=$ all elements.}
...
\begin{proposition} \label{prop:measure}
  Every weight function induces a measure, i.e, for any weight function $W\colon
  2^U \to \mathbb{R}_{\ge 0}$, the function $M_W\colon 2^{2^U} \to
...
\end{proposition}
\begin{proof}
  \todo[inline]{Prove that $M_W$ satisfies the definition of a measure.}
\end{proof}

The process of calculating the value of $M_W(x)$ for some $x \in 2^{2^U}$ with a
given definition of $W$ is known as \emph{weighted model counting}.
...
\begin{example}[Relation to the classical view of WMC] \label{example:construction}
  Let $\mathcal{L}$ be a propositional logic with a set of atoms $U = \{ p, q
  \}$. Let $w : \{ p, q, \neg p, \neg q \} \to \mathbb{R}_{\ge 0}$ be the
  \emph{weight function} defined by
  \begin{align*}
    w(p) = 0.3, \quad w(\neg p) = 0.7, \quad w(q) = 0.2, \quad w(\neg q) = 0.8.
  \end{align*}
  ...
  \begin{equation} \label{eq:wmc_example}
    \mathrm{WMC}(\Delta) = \sum_{\omega \models \Delta} \prod_{\omega \models l} w(l) = w(p)w(q) + w(p)w(\neg q) = 0.3.
  \end{equation}

  Alternatively, we can define $W_p\colon 2^{\{p\}} \to \mathbb{R}_{\ge 0}$ as
  \[
    W_p(\{ p \}) = 0.3, \quad W_p(\emptyset) = 0.7
  \]
  and $W_q\colon 2^{\{q\}} \to \mathbb{R}_{\ge 0}$ as
  \[
    W_q(\{ q \}) = 0.2, \quad W_q(\emptyset) = 0.8.
  \]
  Let $M$ be the measure on $2^{2^U}$ induced by $W = W_p \cdot W_q$. Then,
  equivalently to \cref{eq:wmc_example}, we can write
  \[
    M(p) = W(\{ p, q \}) + W(\{ p \}) = W_p(\{p\})W_q(\{q\}) +
    W_p(\{p\})W_q(\emptyset) = 0.3.
  \]

  Given the theory $\Delta$, we can compute the probability of a query $q$ as
  \cite{DBLP:conf/uai/Belle17,DBLP:conf/aaai/SangBK05}
  \[
    \Pr_{\Delta, w}(q) = \frac{\WMC(\Delta \land q)}{\WMC(\Delta)}.
  \]
  The same thing can be accomplished using the algebraic formulation, with $M$
  replacing $\mathrm{WMC}$.
\end{example}

For the rest of the paper, we let $U$ be the set of (logical) atoms. We use
...
\end{theorem}
\begin{proof}
  \todo[inline]{Check if the proof is valid for zero weights.}
...
\todo[inline]{Reformulate this as a running example throughout the section.}
