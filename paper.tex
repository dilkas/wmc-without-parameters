...
\item Prove that we have an algebra of functions with one additional operation.
...
\item Note that we're going to use logical notation for BAs. Give an example of
  the two notations.
\item Show how the $\phi$ from the algorithm can be encoded in the typical
  DIMACS CNF format.
\end{itemize}
...
\begin{definition}[Operations on functions]
  Let $A\colon 2^X \to [0, 1]$ and $B\colon 2^Y \to [0, 1]$ be Boolean
  functions, $\alpha \in [0, 1]$, and $x \in X$. We define the following
  operations:
  \begin{description}
  \item[Addition:] $A+B$ is a function $A+B\colon 2^{X \cup Y} \to [0, 1]$ such
    that
    \[
      (A+B)(\tau) = A(\tau \cap X) + B(\tau \cap Y)
    \]
    for all $\tau \in 2^{X \cup Y}$.
  \item[Inverse:] $\overline{A}$ is a function $\overline{A}\colon 2^X \to [0,
    1]$ such that
    \[
...
\end{definition}
\begin{remark}
  Note that $A + \overline{A} = 1$.
\end{remark}

\begin{definition}[Special functions]
  \phantom{}
  \begin{itemize}
  \item unit $1\colon 2^\emptyset \to [0, 1]$, $1(\tau) = 1$.
  \item zero $0\colon 2^\emptyset \to [0, 1]$, $0(\tau) = 0$.
  \item constant $[a]\colon 2^{\{a\}} \to [0, 1]$,
    \[
      [a](\tau) =
      \begin{cases}
        1 & \text{if } a \in \tau \\
  ...
  \For{$X \in V$}{
    ...
    \eIf{$|\im X| = 2$}{
      \For{$(y_1, \dots, y_n) \in \prod_{i = 1}^n \im Y_i$}{
        ...
        $\phi \gets \phi \cdot \left( p_1[\lambda_{X=1}] \cdot \prod_{i=1}^n
          [\lambda_{Y_i=y_i}] + p_0 \overline{[\lambda_{X=1}]} \cdot
          \prod_{i=1}^n [\lambda_{Y_i=y_i}] \right)$\;
      }
    }{
      \textit{let} $\im X = \{ x_1, \dots, x_m \}$\;
      $\phi \gets \phi \cdot \left( \sum_{i=1}^m [\lambda_{X = x_i}] \right)
      \cdot \prod_{i=1}^m \prod_{j=i+1}^m (\overline{[\lambda_{X = x_i}]} +
      \overline{[\lambda_{X = x_j}]})$\;
      \For{$x \in \im X$ {\rm \textbf{and}} $(y_1, \dots, y_n) \in \prod_{i = 1}^n \im
        Y_i$}{
        $p_x \gets \Pr(X = x \mid Y_1 = y_1, \dots, Y_n = y_n)$\;
        $\phi \gets \phi \cdot \left( p_x[\lambda_{X=x}] \cdot \prod_{i=1}^n
          [\lambda_{Y_i=y_i}] + \overline{[\lambda_{X=1}]} \cdot \prod_{i=1}^n
          [\lambda_{Y_i=y_i}] \right)$\;
      }
    }
  }
  \Return{$\phi$}\;
...
\begin{lemma} \label{lemma:cpt}
  Let $X \in V$ be a random variable with $\mathrm{pa}(X) = \{ Y_1, \dots, Y_n
  \}$. Then the iteration of the main loop of the algorithm (when run on $X$)
  multiplies $\phi$ by a function $\mathrm{CPT}_X\colon 2^{E^*(X)} \to [0, 1]$
  such that
  \[
    \mathrm{CPT}_X(\tau) =
    \begin{cases}
      \Pr(X = x \mid Y_1 = y_1, \dots, Y_n = y_n) &
      \begin{aligned}
        &\text{if } \textstyle \tau = \lambda_{X=x} \land \bigwedge_{i=1}^n
        \lambda_{Y_i=y_i} \\
        &\text{for some } \textstyle (x, y_1, \dots, y_n) \in \im X \times
        \prod_{i=1}^n \im Y_i
        \end{aligned} \\
      0 & \text{otherwise.}
    \end{cases}
...
\begin{proposition} \label{lemma:full_distribution}
  $\prod_{X \in V} \mathrm{CPT}_X\colon 2^U \to [0, 1]$ represents the full
  probability distribution of the Bayesian network, i.e., if $V = \{ X_1, \dots,
  X_n\}$, then
  \[
    \left( \prod_{X \in V} \mathrm{CPT}_X \right)(\tau) =
    \begin{cases}
...
\begin{proof}
  We have that
  \[
    \left( \prod_{X \in V} \mathrm{CPT}_X \right)(\tau) = \prod_{X \in V}
    \mathrm{CPT}_X(\tau \cap E^*(X)).
  \]
  If $\mathrm{CPT}_X(\tau \cap E^*(X)) = 0$ for some $X \in V$, then $\left(
    \prod_{X \in V} \mathrm{CPT}_X \right)(\tau) = 0$. Otherwise, for each $X
  \in V$, $\tau \cap E^*(X)$ can be expressed as a conjunction of indicator
  variables. Moreover, those indicator variables must match, i.e.,
  \[
    \tau = \bigwedge_{X \in V} \lambda_{X = v_X}
  \]
  for some $(v_X)_{X \in V} \in \prod_{X \in V} \im X$. Then, by
  \cref{lemma:cpt},
  \[
    \left( \prod_{X \in V} \mathrm{CPT}_X \right)(\tau) = \prod_{X \in V} \Pr
    \left( X=v_X \;\middle|\; \bigwedge_{Y \in \mathrm{pa}(X)} Y=v_Y \right) =
    \Pr \left( \bigwedge_{X \in V} X=v_X \right),
  \]
  where the last step follows from the definition of a Bayesian network.
\end{proof}

\begin{theorem}
  Let $\phi\colon 2^U \to [0, 1]$ be a function generated by the algorithm. Then
  \[
...
% Acknowledgments. This work has made use of the resources provided by the
