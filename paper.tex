...
\item When the Bayesian network has an evidence file, we compute the probability
  of evidence. Otherwise, we pick the last (i.e., last to be mentioned) node in
  the Bayesian network to be the goal (the positive value for binary networks
  and the last value in the general case). This applies to the Grid data set (as
...
\begin{align*}
  ...
  &\texttt{w 3 -1 -2 0.7}
\end{align*}
The encoding weights depend on the probabilities in the CPT as in
\[
  w(x_n \mid \neg x_1, \dots, \neg x_{n-1}) = \Pr(X = x_n) \prod_{i=1}^{n-1}
  \frac{1}{1 - w(x_i \mid \neg x_1, \dots, \neg x_{i-1})}.
\]
